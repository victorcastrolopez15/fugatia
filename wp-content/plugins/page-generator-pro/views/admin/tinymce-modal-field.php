...
switch ( $field['type'] ) {
    ...
    case 'autocomplete':
        ...
        <input type="text" 
                ...
                class="widefat wpzinc-autocomplete <?php echo ( isset( $field['class'] ) ? $field['class'] : '' ); ?>" />
        <?php
        break;
    ...
    case 'select_multiple':
        ...
        <select name="<?php echo $field_name; ?>[]"
            ...
            if ( isset( $field['values'] ) && is_array( $field['values'] ) && count( $field['values'] ) > 0 ) {
                foreach ( $field['values'] as $value => $label ) {
                    ?>
                    <option value="<?php echo $value; ?>"<?php echo ( in_array( $value, $field['default_value'] ) ? ' selected' : '' ); ?>>
                        <?php echo $label; ?>
                    </option>
                    <?php
                }
    ...
    case 'toggle':
        ...
        <select name="<?php echo $field_name; ?>"
            ...
            $field['default_value'] = ( isset( $field['default_value'] ) ? $field['default_value'] : '' );
            ?>
            <option value="0"<?php selected( $field['default_value'], 0 ); ?>><?php _e( 'No', 'page-generator-pro' ); ?></option>
            <option value="1"<?php selected( $field['default_value'], 1 ); ?>><?php _e( 'Yes', 'page-generator-pro' ); ?></option>
        </select>
        <?php
        break;
}
