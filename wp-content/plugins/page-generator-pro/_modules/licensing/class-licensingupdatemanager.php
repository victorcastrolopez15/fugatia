...
class LicensingUpdateManager {
	...
	public function __construct( $plugin, $endpoint ) {
		...
		if ( is_admin() ) {
			 ...
			 * Licensing Screen
			 */
			if ( isset( $_GET['page'] ) && sanitize_text_field( $_GET['page'] ) === $this->plugin->name ) { /* phpcs:ignore */
				if ( isset( $_POST[ $this->plugin->name ] ) && is_array( $_POST[ $this->plugin->name ] ) && array_key_exists( 'licenseKey', $_POST[ $this->plugin->name ] ) ) { /* phpcs:ignore */
					update_option( $this->plugin->name . '_licenseKey', sanitize_text_field( $_POST[ $this->plugin->name ]['licenseKey'] ) ); /* phpcs:ignore */
				}
	...
	public function admin_notices() {
		...
		if ( ! $cache['valid'] ) {
			?>
			<div class="notice error"><p><?php echo $cache['message']; // phpcs:ignore ?></p></div>
			<?php
			return;
		}
		...
		if ( $screen->base === 'toplevel_page_' . $this->plugin->name ||
			( isset( $_REQUEST['page'] ) && sanitize_text_field( $_REQUEST['page'] ) === $this->plugin->name ) ) { /* phpcs:ignore */
			?>
			<div class="notice updated"><p><?php echo $cache['message']; // phpcs:ignore ?></p></div>
			<?php
			return;
		}
	...
	public function check_license_key_valid( $force = false ) {
		...
		if ( ! $this->check_license_key_exists() ) {
			$this->cache_set(
				...
				__( '%s: Please specify a license key on the Licensing screen.', $this->plugin->name ), /* phpcs:ignore */
					...
					$message = implode(
						...
						array(
							sprintf(
								...
								__( 'Unable to communicate with the licensing server. %1$s will continue to function, but if this error persists, please open a <a href="%2$s">support request</a> including the following information:', $this->plugin->name ), /* phpcs:ignore */
								$this->plugin->displayName,
								$this->plugin->support_url
							),
							sprintf(
								/* translators: Plugin Name */
								__(' Product Name: %s', $this->plugin->name ), /* phpcs:ignore */
								$this->plugin->displayName
							),
							sprintf(
								/* translators: Plugin Version */
								__(' Product Version: %s', $this->plugin->name ), /* phpcs:ignore */
								$this->plugin->version
							),
							sprintf(
								/* translators: Domain / Site URL */
								__(' Domain: %s', $this->plugin->name ), /* phpcs:ignore */
								$params['site_url']
							),
							sprintf(
								/* translators: Server IP Address */
								__(' IP Address: %s', $this->plugin->name ), /* phpcs:ignore */
		...
		if ( $code !== 200 && $code !== 301 ) {
			...
			$this->cache_set(
				...
				$this->plugin->displayName . ': ' . sprintf(
					__( 'Licensing Server HTTP % s Error . ', $this->plugin->name ), /* phpcs:ignore */
					$code
				)
			);
		...
		$this->cache_set(
			(int) $result->code,
			$this->plugin->displayName . ': ' . (string) $result->codeDescription, /* phpcs:ignore */
			( isset( $result->productVersion ) ? (string) $result->productVersion : 0 ), /* phpcs:ignore */
		...
		$plugin_display_name = $this->get_feature_parameter( 'whitelabelling', 'display_name', $this->plugin->displayName );
		...
		if ( $plugin_display_name !== $this->plugin->displayName ) {
			$this->cache_set(
				(int) $result->code,
				$plugin_display_name . ': ' . (string) $result->codeDescription, /* phpcs:ignore */
				( isset( $result->productVersion ) ? (string) $result->productVersion : 0 ), /* phpcs:ignore */
				( isset( $result->product ) ? $result->product : '' ),
				( isset( $result->features ) ? $result->features : '' ),
				( isset( $result->features_parameters ) ? $result->features_parameters : '' )
	...
	public function plugins_api( $api, $action = '', $args = null ) {
		...
		if ( ! empty( $cache['package'] ) ) {
			...
			if ( ! empty( $changelog_url ) ) {
				$api->sections['changelog'] = '<a href="' . $changelog_url . '" target="_blank">' . __( 'View Changelog', $this->plugin->name ) . '</a>'; /* phpcs:ignore */
			}
