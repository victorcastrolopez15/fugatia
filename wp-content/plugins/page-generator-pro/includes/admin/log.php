...
class Page_Generator_Pro_Log {
	...
	public function upgrade() {
		...
		// Fetch columns.
		$columns = $wpdb->get_results( 'SHOW COLUMNS FROM ' . $wpdb->prefix . $this->table ); // phpcs:ignore
		...
		foreach ( $required_columns as $column => $exists ) {
			...
			switch ( $column ) {
				case 'result':
					$wpdb->query( 'ALTER TABLE ' . $wpdb->prefix . $this->table . ' ADD COLUMN `' . $column . "` enum('success','error') NOT NULL DEFAULT 'success' AFTER `system`" ); // phpcs:ignore
					$wpdb->query( 'ALTER TABLE ' . $wpdb->prefix . $this->table . ' ADD INDEX `' . $column . '` (`' . $column . '`)' ); // phpcs:ignore
					break;
			}
		}
	...
	public function run_log_table_bulk_actions() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-page-generator-pro-logs' ) ) {
		...
		$bulk_action = array_values(
			array_filter(
				array(
					( isset( $_REQUEST['bulk_action'] ) && $_REQUEST['bulk_action'] != '-1' ? sanitize_text_field( $_REQUEST['bulk_action'] ) : '' ), // phpcs:ignore
					( isset( $_REQUEST['bulk_action2'] ) && $_REQUEST['bulk_action2'] != '-1' ? sanitize_text_field( $_REQUEST['bulk_action2'] ) : '' ), // phpcs:ignore
	...
	private function redirect() {
		...
		foreach ( $this->get_filters() as $filter ) {
			if ( ! isset( $_REQUEST[ $filter ] ) ) { // phpcs:ignore
				continue;
			}
			if ( empty( $_REQUEST[ $filter ] ) ) { // phpcs:ignore
				continue;
			}

			$params[ $filter ] = esc_html( $_POST[ $filter ] ); // phpcs:ignore
		}
	...
	public function delete_by_ids( $ids ) {
		...
		return $wpdb->query( 'DELETE FROM ' . $wpdb->prefix . $this->table . ' WHERE id IN (' . implode( ',', array_map( 'absint', $ids ) ) . ')' ); // phpcs:ignore
	...
	public function delete_by_generated_at_cutoff( $date_time ) {
		...
		// Build query.
		$query = $wpdb->prepare( 'DELETE FROM ' . $wpdb->prefix . $this->table . ' WHERE generated_at < %s', $date_time ); // phpcs:ignore
	...
	public function delete_all() {
		...
		return $wpdb->query( 'TRUNCATE TABLE ' . $wpdb->prefix . $this->table ); // phpcs:ignore
	...
	public function get_search_params() {
		...
		$search = esc_html( $_REQUEST['s'] ); // phpcs:ignore
	...
	public function get_group_id() {
		...
		return ( isset( $_REQUEST['group_id'] ) ? absint( $_REQUEST['group_id'] ) : '' ); // phpcs:ignore
	...
	public function get_system() {
		...
		return ( isset( $_REQUEST['system'] ) ? esc_html( $_REQUEST['system'] ) : '' ); // phpcs:ignore
	...
	public function get_result() {
		...
		return ( isset( $_REQUEST['result'] ) ? esc_html( $_REQUEST['result'] ) : '' ); // phpcs:ignore
	...
	public function get_generated_at_start_date() {
		...
		return ( isset( $_REQUEST['generated_at_start_date'] ) ? esc_html( $_REQUEST['generated_at_start_date'] ) : '' ); // phpcs:ignore
	...
	public function get_generated_at_end_date() {
		...
		return ( isset( $_REQUEST['generated_at_end_date'] ) ? esc_html( $_REQUEST['generated_at_end_date'] ) : '' ); // phpcs:ignore
	...
	public function get_order_by() {
		...
		return ( isset( $_GET['orderby'] ) ? sanitize_text_field( $_GET['orderby'] ) : 'generated_at' ); // phpcs:ignore
	...
	public function get_order() {
		...
		return ( isset( $_GET['order'] ) ? sanitize_text_field( $_GET['order'] ) : 'DESC' ); // phpcs:ignore
	...
	public function get_page() {
		...
		return ( ( isset( $_REQUEST['paged'] ) && ! empty( $_REQUEST['paged'] ) ) ? absint( $_REQUEST['paged'] ) : 1 ); // phpcs:ignore
