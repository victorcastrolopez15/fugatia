...
class Page_Generator_Pro_Groups_UI {
	...
	public function output_add_new_buttons( $views ) {
		...
		$views['add_new_directory'] = '<a href="admin.php?page=page-generator-pro-groups-directory" class="page-generator-pro-group-action page-title-action hidden">' . __( 'Add New Directory Structure', 'page-generator-pro' ) . '</a>';
		return $views;
	...
	public function output_posts_filter_by_group_dropdown( $post_type ) {
		...
		$current_group_id = ( isset( $_REQUEST['page_generator_pro_group_id'] ) ? sanitize_text_field( $_REQUEST['page_generator_pro_group_id'] ) : '' ); // phpcs:ignore
	...
	public function posts_filter_by_group( $query ) {
		...
		if ( ! isset( $_REQUEST['page_generator_pro_group_id'] ) ) { // phpcs:ignore
			return $query;
		}
		if ( empty( $_REQUEST['page_generator_pro_group_id'] ) ) { // phpcs:ignore
			return $query;
		}

		// Bail if the filter isn't for the Post Type that we're viewing.
		if ( sanitize_text_field( $_REQUEST['post_type'] ) !== $query->query_vars['post_type'] ) { // phpcs:ignore
			return $query;
		}

		// Get Group ID.
		$group_id = sanitize_text_field( $_REQUEST['page_generator_pro_group_id'] ); // phpcs:ignore
	...
	public function output_meta_box_attributes( $post ) {
		...
		$parent_groups_dropdown = wp_dropdown_pages(
			array(
				'post_type'        => $this->base->get_class( 'post_type' )->post_type_name, // phpcs:ignore
				'exclude_tree'     => $post->ID, // phpcs:ignore
				'selected'         => $post->post_parent, // phpcs:ignore
				'name'             => 'parent_id',
	...
	public function output_meta_box_featured_image( $post ) {
		...
		$wikipedia_languages      = $this->base->get_class( 'wikipedia' )->get_languages();
