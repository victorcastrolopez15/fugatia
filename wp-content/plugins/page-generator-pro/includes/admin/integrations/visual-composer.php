...
class Page_Generator_Pro_Visual_Composer extends Page_Generator_Pro_Integration {
	...
	public function visual_composer_set_current_screen( $result ) {
		...
		if ( ! array_key_exists( 'vcv-action', $_REQUEST ) ) { // phpcs:ignore
			return $result;
		}

		if ( ! array_key_exists( 'post', $_REQUEST ) ) { // phpcs:ignore
			return $result;
		}
		...
		if ( $this->base->plugin->name !== get_post_type( absint( $_REQUEST['post'] ) ) ) { // phpcs:ignore
			return $result;
		}
