...
class Page_Generator_Pro_ListingPro extends Page_Generator_Pro_Integration {
	...
	public function listings_enqueue_scripts() {
		...
		wp_enqueue_script( 'accordianui', plugins_url( 'assets/js/jqueryuiaccordian.js', dirname( dirname( __FILE__ ) ) ), 'jquery', $this->base->plugin->version, true );
		wp_enqueue_script( 'jquery-ui', get_template_directory_uri() . '/assets/js/jquery-ui.js', 'jquery', $this->base->plugin->version, true );
		wp_enqueue_script( 'jquery-ui-trigger', get_template_directory_uri() . '/assets/js/jquery-ui-trigger.js', 'jquery', $this->base->plugin->version, true );
		wp_localize_script( 'jquery-ui-trigger', 'global', array( 'ajax' => admin_url( 'admin-ajax.php' ) ) );
		wp_enqueue_script( 'jquery-droppin', get_template_directory_uri() . '/assets/js/drop-pin.js', 'jquery', $this->base->plugin->version, true );
	...
	public function save( $group_id, $settings, $request ) {
		...
		global $listingpro_settings, $reviews_options, $listingpro_formFields, $claim_options, $ads_options, $page_options, $post_options, $price_plans_options; // phpcs:ignore
	...
	public function save_event( $group_id, $settings, $request ) {
		...
		$event_id         = sanitize_text_field( $request['event-lsiting-id'] ); // Misspelling is deliberate.
