...
class Page_Generator_Pro_Divi_Module extends ET_Builder_Module {
	...
	public function init() {
		...
		// Get shortcode.
		$this->shortcode = Page_Generator_Pro()->get_class( 'shortcode' )->get_shortcode( str_replace( 'page-generator-pro-divi-', '', $this->slug ) );
		if ( ! $this->shortcode ) {
			return;
		}

		// Define name.
		$this->name = '_' . $this->shortcode['title'];
		...
		foreach ( $this->shortcode['fields'] as $field_name => $field ) {
			...
			if ( isset( $field['condition'] ) ) {
				...
				if ( $this->shortcode['fields'][ $field['condition']['key'] ]['type'] === 'toggle' ) {
					$value = ( ( $value == '1' ) ? 'on' : 'off' ); // phpcs:ignore
				}
			...
			switch ( $field['type'] ) {
				...
				case 'select_multiple':
					$fields[ $field_name ]['type'] = 'text'; // @TODO add support for these field types in Divi.
					break;
	 ...
	 * @since   3.0.7
	 *
	 * @param   mixed  $unprocessed_props  Unprocessed properties.
	 * @param   mixed  $content            Content.
	 * @param   string $render_slug        Slug.
	 */
	public function render( $unprocessed_props, $content, $render_slug ) { // phpcs:ignore
		...
		return Page_Generator_Pro()->get_class( 'shortcode' )->build_shortcode(
			Page_Generator_Pro()->plugin->name . '-' . str_replace( 'page-generator-pro-divi-', '', $this->slug ),
