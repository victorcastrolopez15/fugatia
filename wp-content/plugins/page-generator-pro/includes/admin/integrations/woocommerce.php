...
class Page_Generator_Pro_WooCommerce extends Page_Generator_Pro_Integration {
	...
	public function __construct( $base ) {
		...
		// Content Groups: Set product_type Taxonomy Term.
		add_action( 'page_generator_pro_generate_content_finished', array( $this, 'generate_content_set_product_type' ), 10, 3 );
	...
	public function enqueue_product_js() {
		...
		// Enqueue woocommerce.js integration to remove validation on some fields.
	 ...
	 * @param   int $post_id    Post ID.
	 */
	private function localize_woocommerce_admin_meta_boxes( $post_id ) { // phpcs:ignore
	 ...
	 * @since   3.3.9
	 *
	 * @param   mixed $product_type   Product Type (false | simple | external).
	 * @param   int   $product_id     Product ID.
	 * @return  mixed                 Product Type
	 */
	...
	public function save_product( $post_id, $post ) {
		...
		add_action( 'woocommerce_process_product_meta', array( $this, 'save_product_prices' ), 30, 2 );

		// @TODO Read price into fields when non-numeric.
	...
	/**
	 * Manually save product prices to the post meta table, as WooCommerce will strip the
	 * Keyword from some fields due to it not matching the expected format.
	 *
	 * @since   3.6.4
	 *
	 ...
	 */
	public function save_product_prices( $post_id, $post ) { // phpcs:ignore

		update_post_meta( $post_id, '_price', $_REQUEST['_regular_price'] ); // phpcs:ignore
		update_post_meta( $post_id, '_sale_price', $_REQUEST['_sale_price'] ); // phpcs:ignore
	 ...
	 * @param   array $settings       Group Settings.
	 */
	public function generate_content_set_product_type( $post_id, $group_id, $settings ) {
