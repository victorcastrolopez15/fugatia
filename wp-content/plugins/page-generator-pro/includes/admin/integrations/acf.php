...
class Page_Generator_Pro_ACF extends Page_Generator_Pro_Integration {
	...
	private function get_acf_content_groups() {
		...
		if ( ! $acf_field_groups || ! count( $acf_field_groups ) ) {
			return false;
		}
	...
	public function prevent_post_meta_copy_to_generated_content( $ignored_keys, $post_id, $settings, $post_args ) { // phpcs:ignore
		...
		foreach ( $acf_content_groups as $group_key => $label ) {
			...
			$overwrite = ( isset( $post_args['ID'] ) && ! array_key_exists( 'acf_' . $group_key, $settings['overwrite_sections'] ) ? false : true );
			if ( $overwrite ) {
				continue;
			}
	...
	public function match_term_group_location_rule( $match, $rule, $options, $field_group = false ) {
		...
		if ( ! isset( $_REQUEST['tag_ID'] ) ) { // phpcs:ignore
			return $match;
		}
		...
		$taxonomy = sanitize_text_field( $_REQUEST['taxonomy'] ); // phpcs:ignore
		$term_id  = absint( sanitize_text_field( $_REQUEST['tag_ID'] ) ); // phpcs:ignore
