...
class Page_Generator_Pro_Yoast_SEO extends Page_Generator_Pro_Integration {
	...
	public function wpseo_sanitize_post_meta__yoast_wpseo_canonical( $clean, $meta_value ) {
		...
		// Get Post ID.
		$group_id = absint( $_POST['ID'] ); // phpcs:ignore
	 ...
	 */
	public function groups_terms_set_term_meta( $term_id, $group_id, $term_meta, $settings, $term_args ) { // phpcs:ignore
		...
		if ( empty( $options_term_meta ) || ! count( $options_term_meta ) ) {
			return;
		}
