...
class Page_Generator_Pro_Keywords_Source_Local {
					...
					'options' => array(
						...
						'file'      => array(
							...
							'description' => array(
								...
								sprintf(
									/* translators: Link to Keywords > Import CSV */
									__( 'Keyword data in a CSV file? Use the %s option.', 'page-generator-pro' ),
									'<a href="' . admin_url( 'admin.php?page=page-generator-pro-keywords&cmd=form-import-file' ) . '">' . __( 'Import CSV', 'page-generator-pro' ) . '</a>'
								),
							),
						),
					),
	 ...
	 * @since   3.0.8
	 *
	 * @return  mixed   false | Path and Filename
	 */
	private function has_text_file() {
		...
		if ( ! ( ! empty( $_FILES[ $this->get_name() ]['type']['file'] ) && preg_match( '/(text|txt)$/i', $_FILES[ $this->get_name() ]['type']['file'] ) ) &&
			! preg_match( '/(text|txt)$/i', $_FILES[ $this->get_name() ]['name']['file'] ) ) {
			return false;
		}

		// Return path and file.
		return $_FILES[ $this->get_name() ]['tmp_name']['file'];
