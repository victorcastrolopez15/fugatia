...
class Page_Generator_Pro_Groups_Table {
	...
	public function admin_columns_output( $column_name, $post_id ) {
			...
			case 'status':
				if ( $this->base->get_class( 'groups' )->generates_content( $post_id ) ) {
					if ( $this->base->get_class( 'groups' )->is_idle( $post_id ) ) {
						$items = $this->base->get_class( 'groups' )->get_actions_links( $post_id );
					} else {
						$items = array(
		...
		foreach ( $items as $class => $item ) {
			echo '<span class="' . $class . '">' . $item . '</span><br />'; // phpcs:ignore
		}
	...
	public function run_row_actions() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'action-page-generator-pro-content-group' ) ) {
			return;
		}
		...
		if ( sanitize_text_field( $_REQUEST['post_type'] ) !== $this->base->get_class( 'post_type' )->post_type_name ) {
			return;
		}
	 ...
	 * @param   mixed  $post_type  Target Post Type.
