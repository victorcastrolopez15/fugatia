...
class Page_Generator_Pro_Groups_Terms_Table {
		...
		switch ( $column_name ) {
			...
			case 'status':
				if ( $this->base->get_class( 'groups_terms' )->is_idle( $term_id ) ) {
					$items = array(
						'test'            => '<a href="' . $this->get_action_url( $term_id, 'test' ) . '">' . $this->base->get_class( 'groups_terms_ui' )->get_title( 'test' ) . '</a>',
	...
	public function run_row_actions() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'action-page-generator-pro-term-group' ) ) {
			return;
		}
		...
		if ( ! isset( $_REQUEST[ $this->base->plugin->name . '-action' ] ) ) { // phpcs:ignore
			return;
		}

		// Fetch action and group ID.
		$action = sanitize_text_field( $_REQUEST[ $this->base->plugin->name . '-action' ] ); // phpcs:ignore
	...
	private function get_search() {
		...
		return ( isset( $_GET['s'] ) ? sanitize_text_field( urldecode( $_GET['s'] ) ) : '' ); // phpcs:ignore
	...
	private function get_order_by() {
		...
		return ( isset( $_GET['orderby'] ) ? sanitize_text_field( $_GET['orderby'] ) : '' ); // phpcs:ignore
