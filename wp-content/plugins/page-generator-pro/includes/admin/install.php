...
class Page_Generator_Pro_Install {
			...
			if ( $table_exists === $wpdb->prefix . 'page_generator_groups' ) {
				...
				if ( is_array( $groups ) && count( $groups ) > 0 ) {
					foreach ( $groups as $group ) {
						...
						$settings = unserialize( $group->settings ); // phpcs:ignore
	...
	public function copy_mu_plugin( $force = true ) {
		...
		if ( ! @file_exists( WPMU_PLUGIN_DIR ) ) { // phpcs:ignore
			$result = @mkdir( WPMU_PLUGIN_DIR ); // phpcs:ignore
			if ( ! $result ) {
				return new WP_Error(
		...
		if ( @file_exists( $mu_plugin_destination ) && ! $force ) { // phpcs:ignore
			return;
		}

		// Delete the existing mu-plugins file if it exists and we're forcing an update.
		if ( @file_exists( $mu_plugin_destination ) && $force ) { // phpcs:ignore
			$result = @unlink( $mu_plugin_destination ); // phpcs:ignore
			if ( ! $result ) {
				return new WP_Error(
					'page_generator_pro_install_copy_mu_plugin',
					sprintf(
						/* translators: Folder and file path */
						__( 'Could not delete %s.', 'page-generator-pro' ),
		...
		$result = @copy( $mu_plugin_source, $mu_plugin_destination ); // phpcs:ignore
		if ( ! $result ) {
			return new WP_Error(
				'page_generator_pro_install_copy_mu_plugin',
				sprintf(
