...
class Page_Generator_Pro_Phone_Area_Codes {
	...
	public function activate() {
		...
		$count = $wpdb->get_var( 'SELECT COUNT(*) FROM ' . $wpdb->prefix . $this->table ); // phpcs:ignore
		if ( $count > 0 ) {
			return true;
		}
		...
		$data = @file_get_contents( 'http://www.aggdata.com/download_sample.php?file=globalareacodes.csv' ); // phpcs:ignore
	...
	public function get_phone_area_codes( $country ) {
		...
		// Get records.
		$query   = $wpdb->prepare(
			'SELECT area_code, city, country_code FROM ' . $wpdb->prefix . $this->table . " WHERE country = %s", // phpcs:ignore
			$country
		);
		$results = $wpdb->get_results( $query, ARRAY_A ); // phpcs:ignore
