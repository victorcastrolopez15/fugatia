...
class Page_Generator_Pro_Shortcode {
	...
	public function maybe_load_js() {
		...
		foreach ( $shortcodes_requiring_js as $shortcode_name ) {
			...
			// Fetch Frontend JS.
			$plugin_js = file_get_contents( $this->base->plugin->folder . '/assets/js/min/frontend-min.js' ); // phpcs:ignore
			...
			// Output.
			echo '<script>' . $plugin_js . '</script>'; // phpcs:ignore
			return;
	...
	private function append_css_to_customizer_css( $customizer_css ) {
		...
		// Fetch Frontend CSS.
		$plugin_css = file_get_contents( $this->base->plugin->folder . '/assets/css/frontend.css' ); // phpcs:ignore
