...
class Page_Generator_Pro_Spintax {
	...
	/**
	 * Searches the given content for any words that are in our array of
	 * spintax words, replacing them with spintax.
	 *
	 * @since   1.7.9
	 *
	...
	public function add_spintax( $content ) {
		...
		switch ( $provider ) {
			 ...
			 */
			case 'chimprewriter':
				$spintax_content = $this->add_spintax_chimprewriter( $content, $protected_words );
				break;
			 ...
			 */
			case 'spinnerchief':
				$spintax_content = $this->add_spintax_spinnerchief( $content, $protected_words );
				break;
			 ...
			 */
			case 'spin_rewriter':
				$spintax_content = $this->add_spintax_spin_rewriter( $content, $protected_words );
				break;
			 ...
			 */
			case 'wordai':
				$spintax_content = $this->add_spintax_wordai( $content, $protected_words );
				break;
		...
		if ( is_wp_error( $spintax_content ) ) {
	 ...
	 * @return  mixed                       WP_Error | string
	 */
	private function add_spintax_chimprewriter( $content, $protected_words ) {
	 ...
	 * @return  mixed                       WP_Error | string
	 */
	private function add_spintax_spinnerchief( $content, $protected_words ) {
	 ...
	 * @return  mixed                       WP_Error | string
	 */
	private function add_spintax_spin_rewriter( $content, $protected_words ) {
	 ...
	 * @return  mixed                       WP_Error | string
	 */
	private function add_spintax_wordai( $content, $protected_words ) {
	...
	private function add_spintax_local( $content, $protected_words ) {
		...
		foreach ( $this->get_synonyms() as $synonyms ) {
			...
			foreach ( $synonyms as $word ) {
				...
				if ( in_array( $word, $protected_words ) ) { // phpcs:ignore
					continue;
				}
